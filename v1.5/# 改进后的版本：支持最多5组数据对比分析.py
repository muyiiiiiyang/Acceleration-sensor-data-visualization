# 改进后的版本：支持最多5组数据对比分析（Time 归一化从 0 开始）
...
class PlotWindow(tk.Toplevel):
    def __init__(self, master, datasets):
        ...
        self.figure, self.axes = plt.subplots(3, 1, figsize=(10, 8), sharex=True, constrained_layout=True)
    ...
    def plot_all_data(self):
        for dataset in self.datasets:
            ...
            df["Time (s)"] -= df["Time (s)"].iloc[0]  # 将时间归一化为从0开始
            time = df["Time (s)"]
...
class ControlPanel(tk.Tk):
    ...
    def plot_all(self):
        ...
        for frame in self.frames:
            ...
            try:
                df = pd.read_csv(path)
                df["Time (s)"] -= df["Time (s)"].iloc[0]  # 所有数据起点归零
                time = df["Time (s)"]
